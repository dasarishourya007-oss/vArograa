...
const uploadFile = async (path, file, onProgress) => {
    ...
    return new Promise((resolve, reject) => {
        const timeout = setTimeout(() => {
            uploadTask.cancel();
            reject(new Error('Upload timed out. Please check network and retry.'));
        }, UPLOAD_TIMEOUT_MS);
...
export const uploadDoctorPhoto = async (doctorId, file, onProgress) => {
    ...
    const downloadURL = await uploadFile(path, preparedFile, onProgress);
    await Promise.all([
...
export const uploadHospitalPhoto = async (hospitalId, file, onProgress) => {
