...
export const AuthProvider = ({ children }) => {
    const [user, setUser] = useState(() => getStoredUser() || { uid: 'demo-user', email: 'admin@varogra.com' });
    const [role, setRole] = useState(() => getStoredUser()?.role || 'Admin');
        ...
        const unsubscribe = onAuthStateChanged(auth, async (authUser) => {
            try {
                if (authUser) {
                    ...
                    const nextRole = userData?.role || hospitalData?.role || cached?.role || 'Admin';
                    setRole(nextRole);
                    ...
                    if (cached) {
                        setUser(cached);
                        setRole(cached?.role || localStorage.getItem('userRole') || 'Admin');
                    } else {
                        console.log('vArogra: No user detected, staying in demo mode.');
                        setUser({ uid: 'demo-user', email: 'admin@varogra.com' });
                        setRole('Admin');
                    }
                }
            } catch (error) {
